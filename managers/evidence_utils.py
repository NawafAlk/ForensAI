"""
Evidence image handler for forensic disk image access via pytsk3 and pyewf.

Provides the ImageHandler class which wraps pytsk3 (The Sleuth Kit) and pyewf
for reading E01/EWF and RAW/dd disk images. Supports partition enumeration,
directory listing, file content extraction, and registry hive access.
"""
...
def validate_magic_bytes(image_path: str, image_type: str) -> bool:
...
class LRUCache:
    ...
    def put(self, key, value):
        ...
        if key in self._cache:
            ...
            self._current_size -= len(old_value) if isinstance(old_value, (bytes, bytearray)) else 0

        while self._current_size + value_size > self._max_size and self._cache:
            _, evicted = self._cache.popitem(last=False)
            self._current_size -= len(evicted) if isinstance(evicted, (bytes, bytearray)) else 0
...
class ImageHandler:
    ...
    def read(self, offset, size):
        ...
        if size > MAX_FILE_READ_SIZE:
            raise ValueError(f"Read size {size} exceeds maximum allowed {MAX_FILE_READ_SIZE}")
        ...
        if hasattr(self.img_info, 'read'):
            try:
                data = self.img_info.read(offset, size)
                self._read_cache.put(cache_key, data)
                return data
                ...
                for entry in directory:
                    if entry.info.name.name not in [b".", b".."]:
                        ...
                        try:
                            ...
                            name = str(entry.info.name.name) if hasattr(entry.info.name, 'name') else None
    ...
    def get_cache_stats(self):
        ...
        return {
            'read_cache': self._read_cache.stats,
            'dir_cache_entries': len(self._dir_cache),
            'fs_info_cache_entries': len(self.fs_info_cache)
        }
    ...
    def get_file_content(self, inode_number, offset):
        """Read file content with sandboxed parsing and resource limits."""
        ...
        try:
            ...
            if file_obj.info.meta is None:
                print("File metadata not available!")
                return None, None
            ...
            print(f"I/O error reading file (inode {inode_number}): {e}")
            return None, None
        except OverflowError as e:
            print(f"File size overflow (inode {inode_number}): {e}")
            return None, None
        except Exception as e:
