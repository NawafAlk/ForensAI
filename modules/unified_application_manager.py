"""
Unified multi-format file viewer for forensic evidence inspection.

Provides integrated viewers for images (JPEG, PNG, BMP, AVIF, etc.),
PDF documents (via PyMuPDF/fitz), audio/video (via Qt Multimedia),
and Office documents. Supports zoom, rotation, and print for applicable
formats.
"""
...
from PySide6.QtGui import QIcon, QPixmap, QImage, QAction, QPageLayout, QDesktopServices
...
class PictureViewer(QWidget):
    ...
    def _load_with_pil(self, content):
    ...
    def rotate_left(self):
        ...
        if pixmap and not pixmap.isNull():
            ...
            self.image_label.setPixmap(pixmap.transformed(transform))
    ...
    def display(self, content):
        ...
        if content:
            try:
                ...
                self.page_label.setText("Unable to load PDF.\nFile may be corrupted.")
        else:
...
class OfficeViewer(QWidget):
    """Viewer for Office documents - opens files with system default application"""
    ...
    def initialize_ui(self):
        ...
        self.open_button = QPushButton("Open in Default Application")
    ...
    def display(self, file_path, file_extension):
        ...
        self.file_type_label.setText(f"File type: {file_extension.upper()} document")
    ...
    def open_in_default_app(self):
        """Open the office file with system default application"""
        if self.current_temp_file:
            try:
                QDesktopServices.openUrl(QUrl.fromLocalFile(self.current_temp_file))
                print(f"Opening office file: {self.current_temp_file}")
            except Exception as e:
                print(f"Error opening office file: {e}")
    ...
    def clear(self):
        ...
        self.info_label.setText("Office document detected")
        self.open_button.setEnabled(False)
